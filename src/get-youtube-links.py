...
cur.execute("SELECT id, artist, title FROM chart_songs WHERE youtube_link IS NULL")
songs = cur.fetchall()
...
for song_id, artist, title in songs:
    ...
    if youtube_link:
        cur.execute("UPDATE chart_songs SET youtube_link = ? WHERE id = ?", (youtube_link, song_id))
        conn.commit()
